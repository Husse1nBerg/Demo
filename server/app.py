    ...
    with get_db_connection() as conn:
        ...
        if cursor.fetchone()['count'] == 0:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?), (?, ?, ?, ?), (?, ?, ?, ?)
            ''', (hotel_id, 'Booking.com', 0.18, 0.25,
                  hotel_id, 'Expedia', 0.20, 0.20,
                  hotel_id, 'Hotels.com', 0.15, 0.15))
...
def get_price_recommendation():
    ...
    try:
        ...
        if not competitors:
            logger.warning("No competitor data available, attempting alternative sources...")
        ...
        logger.info(f"Recommendation generated with {pricing_result['confidence_score']*100:.1f}% confidence")
    ...
    try:
        ...
        if desired_rank <= len(sorted_competitors):
            if desired_rank == 1:
                ...
                lower_price = sorted_competitors[desired_rank - 1]['price'] if desired_rank - 1 < len(sorted_competitors) else higher_price * 0.95
        ...
        avg_competitor_price = sum(c['price'] for c in competitors) / len(competitors) if competitors else target_price
        price_ratio = target_price / avg_competitor_price if avg_competitor_price > 0 else 1.0
...
def demand_forecast():
    ...
    try:
        ...
        if not forecast or len(forecast) < 7:
            logger.warning("Insufficient forecast data, generating based on historical patterns")
        ...
        return jsonify({"success": True, "forecast": forecast[:7]})  # Return 7 days for UI
...
def direct_booking_intelligence():
    ...
    try:
        ...
        logger.error(f"Error in direct booking intelligence endpoint: {e}", exc_info=True)
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_historical_performance_data():
    ...
    try:
        ...
        logger.info(f"Fetching {days} days of historical performance for {location_str}")
        ...
        if not performance_data.get('history'):
            ...
            performance_data = analytics.generate_historical_data_from_sources(location_str, days)
        
        logger.info(f"Returning {len(performance_data.get('history', []))} historical data points")
